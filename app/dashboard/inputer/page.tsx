...
export default function ImputerPerformancePage() {
  ...
  // Update charts when selected Imputers change
  useEffect(() => {
    updateBarChart()
    updatePieChart()
    updateAchChart()
  ...
  const updateBarChart = () => {
    ...
    const sortedData = [...selectedImputersData].sort((a, b) => b.total - a.total)

    new Chart(ctx, {
      type: "bar",
      data: {
        labels: sortedData.map((imputer) => imputer.name.split(",")[0]), // Use first part of name
        datasets: [
          {
            label: "Total Orders",
            data: sortedData.map((imputer) => imputer.total),
  ...
  const updatePieChart = () => {
    ...
    // Hitung persentase complete dari semua Imputers yang dipilih
    const totalCompletionsAll = selectedImputersData.reduce((sum, imputer) => sum + imputer.complete, 0)
    // Urutkan selectedImputersData dari persentase terbesar
    const sortedImputersForPie = [...selectedImputersData].sort((a, b) => {
    ...
    const labels = sortedImputersForPie.map((imputer) => imputer.name.split(",")[0]);
    const dataPie = sortedImputersForPie.map((imputer) => totalCompletionsAll > 0 ? (imputer.complete / totalCompletionsAll) * 100 : 0);
  ...
  const updateAchChart = () => {
    ...
    new Chart(ctx, {
      type: "bar",
      data: {
        labels: sortedData.map((imputer) => imputer.name.split(",")[0]),
        datasets: [
          {
            type: "bar",
            label: "Total",
      ...
      <div className="p-6 space-y-6 bg-gray-50 flex-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-3">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              ...
              <Card>
                <CardContent className="p-4">
                  <h2 className="text-lg font-medium mb-4">Imputer Distribution</h2>
        ...
        <Card>
          <CardContent className="p-4 prose max-w-none">
            <h2 className="text-lg font-medium">Insight Imputer</h2>
            <p className="text-base">{insightImputer}</p>
          </CardContent>
        </Card>
