...
// Interface untuk data Order Age
interface OrderAgeData {
  name: string
  total: number
  failed: number
  complete: number
...
// Fungsi untuk fetch data UMUR ORDER dari sumber data yang dipilih user
async function fetchUmurOrderData(userId: string) {
  const result = await fetchDataFromSource(userId, 'UMUR ORDER NCX');
...
// Fungsi untuk fetch insight UMUR ORDER dari sumber data yang dipilih user
async function fetchInsightUmurOrder(userId: string) {
...
export default function OrderAgePage() {
  ...
  const [umurOrderData, setUmurOrderData] = useState<OrderAgeData[]>([])
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const [umurOrderDataResult, insightUmurOrderResult] = await Promise.all([
          fetchUmurOrderData(user.id), fetchInsightUmurOrder(user.id)
        ])
