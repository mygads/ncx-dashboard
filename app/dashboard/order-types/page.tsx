...
export default function OrderTypesPage() {
  ...
  // Sort selected orders data by total orders descending (for all charts)
  const sortedSelectedOrdersData = [...orderData.filter((o) => selectedOrders.includes(o.name))].sort((a, b) => b.total - a.total)

  // Data for Distribution (Bar Only)
  const barOnlyChartData = sortedSelectedOrdersData.map((o, i) => ({
    label: o.name,
    value: o.total,
  ...
  const totalCompletionsAll = sortedSelectedOrdersData.reduce((sum, o) => sum + o.complete, 0)
  const pieChartData = sortedSelectedOrdersData.map((o, i) => ({
    status: o.name,
  ...
  const barChartData = sortedSelectedOrdersData.map((o) => ({
    month: o.name,
    totalOrders: o.total,
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-6 space-y-6 flex-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-3">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              <CloneBarOnlyChart data={barOnlyChartData} title="Order Type Distribution" />
              <ClonePieChart data={pieChartData} title="Complete (%)" />
            </div>
            <CloneBarChart data={barChartData} title="Order Type Achievement" />
          </div>
        ...
        <CloneInsightCard title="Insight Tipe Order" text={insightOrder} />
        <LastUpdatedFooter />
      </div>
    </div>
  )
